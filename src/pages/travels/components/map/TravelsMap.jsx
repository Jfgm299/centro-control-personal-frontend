...
export default function TravelsMap({
  ...
  const markerGroups = useMemo(() => {
    ...
    trips.filter(t => t.lat && t.lon).forEach(trip => {
      ...
      if (!groups.has(key)) groups.set(key, { coords: [trip.lon, trip.lat], trips: [] })
  ...
  const handleMoveEnd = useCallback(({ zoom: z, coordinates }) => {
    setZoom(z); setCenter(coordinates); setActiveGroup(null)
  }, [])

  const handleZoomIn  = () => setZoom(z => Math.min(z * 2, MAX_ZOOM))
  const handleZoomOut = () => setZoom(z => Math.max(z / 2, MIN_ZOOM))
  const handleReset   = () => { setZoom(initialZoom); setCenter(initialCenter); setActiveGroup(null) }
    ...
    <div style={{ background: `linear-gradient(180deg, ${MAP_COLORS.ocean} 0%, ${MAP_COLORS.oceanGradient} 100%)` }}>
      {visitedCountryCodes.length > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 border-b border-white/10 overflow-x-auto">
          <span className="text-white/40 text-xs font-mono shrink-0">{visitedCountryCodes.length} countries</span>
          <div className="flex gap-1.5">
            {visitedCountryCodes.map(code => <CircularFlag key={code} code={code} size={22} />)}
          </div>
        </div>
      )}

      <div ref={containerRef} style={{ position: 'relative', height, overflow: 'hidden' }}>
        <ComposableMap projection="geoNaturalEarth1" projectionConfig={{ scale: 140, center: [0, -40] }} style={{ width: '100%', height: 'auto', display: 'block' }}>
              ...
              {({ geographies }) => geographies.map(geo => {
                ...
                return (
                  <Geography key={geo.rsmKey} geography={geo}
                    ...
                    stroke={MAP_COLORS.landStroke} strokeWidth={0.4 / zoom}
                    style={{ default: { outline: 'none' }, hover: { outline: 'none', fill: isVisited ? MAP_COLORS.visitedHover : '#4a2aaa' }, pressed: { outline: 'none' } }}
                  />
                )
              })}
            </Geographies>
            {markerGroups.map((group, i) => (
              <Marker key={i} coordinates={group.coords} onClick={() => setActiveGroup(group)} style={{ cursor: 'pointer' }}>
                <circle r={markerRadius * (group.trips.length > 1 ? 1.4 : 1)} fill={MAP_COLORS.marker} stroke={MAP_COLORS.markerStroke} strokeWidth={strokeWidth} opacity={0.95} />
                {group.trips.length > 1 && (
                  <text textAnchor="middle" dominantBaseline="central" style={{ fontSize: markerRadius * 0.9, fill: '#1a0550', fontWeight: 'bold', pointerEvents: 'none' }}>
                    {group.trips.length}
                  </text>
                )}
              </Marker>
        ...
        {activeGroup && <TripMarker group={activeGroup} onClose={() => setActiveGroup(null)} onTripClick={onTripClick} />}
