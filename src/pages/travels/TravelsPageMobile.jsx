...
export default function TravelsPageMobile() {
  ...
  return (
    <div className="pb-32">
      ...
      {/* World map — altura reducida + zoom inicial en Europa */}
      {activeTab === 'map' && (
        ...
        {activeTab === 'map' && (
          <div className="space-y-3">
            ...
            {stats.tripsWithCoords.length === 0 && stats.totalTrips > 0 && (
              <p className="text-center text-gray-400 text-sm py-4">{t('map.noCoords')}</p>
            )}
            {stats.totalTrips === 0 && (
              <p className="text-center text-gray-400 text-sm py-4">{t('map.empty')}</p>
            )}
          </div>
        )}
